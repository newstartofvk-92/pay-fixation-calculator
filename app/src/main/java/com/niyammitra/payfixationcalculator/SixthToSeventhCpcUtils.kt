 ...
 * basic pay is Pay in Pay Band + Grade Pay. Medical officers entitled to NPA/practice allowance require a separate Rule 7 treatment and are not silently routed through this calculation.
 */
data class SixthCpcPayBand(
    val title: String,
    val gradePays: List<Int>,
    val payBandMaximum: Int = when (title.substringBefore(":")) {
        "PB-1" -> 20200
        "PB-2" -> 34800
        "PB-3" -> 39100
        "PB-4" -> 67000
...
private val amendedLevel13Stages = listOf(123100, 126800, 130600, 134500, 138500, 142700, 147000, 151400, 155900, 160600, 165400, 170400, 175500, 180800, 186200, 191800, 197600, 203500, 209600, 215900)
...
fun calculateSixthToSeventhCpc(payInPayBand: Int, gradePay: Int, payBand: SixthCpcPayBand): SixthToSeventhResult? {
